...
/** Generator for synthetic, clustered data running on Spark.
  *
  * The data is generated by random uniform sampling around the given
  * centers and adding gaussian noise to each dimension.
 *
 * @param master Spark master
 * @param numTasks Degree of parallelism
 * @param pointsPerTask Number of dataPoints per tasks to generate
 * @param input Input file-path holding the cluster centers and the variance for the Gaussian
 * @param output Output file-path where the resulting datapoints are saved
 */
